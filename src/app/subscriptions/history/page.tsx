...
export default function PaymentHistoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // ── Payments: query the live payments table directly ──────────────────
      ...
      if (!paymentsError && paymentsData) {
        const mapped: PaymentRequest[] = paymentsData.map((p: any) => ({
          id: p.id,
          userId: p.user_id,
          packageType: p.package_type,
          packageCategory: p.package_category,
          gameId: p.game_id,
          ...
          contentDurationDays: p.content_duration_days,
        }));
        setPayments(mapped);
      }

      // ── Subscriptions: still read from user object (updated on auth) ──────
      const allSubs = [
