...
import { Tv, Play, Star, Clock, Users, Search, Filter, Grid, List } from 'lucide-react';
import { motion } from 'framer-motion';
import { subscribeToSeries } from '@/lib/content-management';
...
export default function SeriesPage() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToSeries((data) => {
      // Filter only active series
      const activeSeries = data.filter(series => series.isActive);
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(series =>
        ...
        series.genre.some(g => g.toLowerCase().includes(searchTerm))
      );
    }
    ...
    if (genreFilter !== 'all') {
      filtered = filtered.filter(series =>
        series.genre.some(g => g.toLowerCase() === genreFilter.toLowerCase())
      );
    }

    setFilteredSeries(filtered);
  }, [series, searchQuery, genreFilter]);

  // Get unique genres for filter
  const genres = Array.from(new Set(series.flatMap(series => series.genre)));
        ...
        <div className="container-mobile space-y-8">
          ...
          <div className="text-center py-6">
            ...
            <p className="text-responsive-base text-dark-300">
              Binge-watch amazing series from East Africa
            </p>
          </div>
          ...
          {filteredSeries.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-dark-200 mb-2">
                {series.length === 0 ? 'No Series Available' : 'No Series Found'}
              </h3>
            ...
            <div className="relative">
              {/* Blurred Background Watermark - Uses first series thumbnail */}
              {filteredSeries[0]?.thumbnailUrl && (
                <div 
                  className="absolute inset-0 -z-10 opacity-[0.04] pointer-events-none rounded-2xl overflow-hidden"
                  style={{
                    backgroundImage: `url(${filteredSeries[0].thumbnailUrl})`,
              ...
              {filteredSeries.map((seriesItem, index) => (
                <Link key={seriesItem.id} href={`/series/${seriesItem.id}`}>
                  ...
                  >
                  {/* Series Thumbnail - Auto-playing Video Preview */}
                  <div className={`relative ${viewMode === 'list' ? 'w-32 h-20 flex-shrink-0' : 'aspect-video'}`}>
                    <VideoThumbnail
                      videoUrl={seriesItem.seasons?.[0]?.episodes?.[0]?.videoUrl || seriesItem.seasons?.[0]?.episodes?.[0]?.googleDriveUrl || seriesItem.seasons?.[0]?.videoUrl || seriesItem.seasons?.[0]?.googleDriveUrl || ''}
                    ...
                    <div className="absolute top-2 right-2">
                      <div className="flex items-center space-x-1 bg-black/50 rounded px-2 py-1">
                        ...
                        <span className="text-xs text-white">{seriesItem.rating || 0}</span>
                      </div>
                    </div>
                    <div className="absolute bottom-2 right-2">
                      <div className="flex items-center space-x-1 bg-black/50 rounded px-2 py-1">
                        <Tv size={12} className="text-white" />
                        <span className="text-xs text-white">{seriesItem.totalSeasons} season{seriesItem.totalSeasons > 1 ? 's' : ''}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className={`p-4 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {seriesItem.genre.slice(0, 3).map((g, idx) => (
                    ...
                    <div className="flex items-center justify-between text-sm text-dark-400 mb-3">
                      <div className="flex items-center space-x-1">
                        <Users size={14} />
                        <span>{seriesItem.views.toLocaleString()}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <Tv size={14} />
                        <span>{seriesItem.totalSeasons} season{seriesItem.totalSeasons > 1 ? 's' : ''}</span>
                      </div>
                    </div>

                    <div className="flex items-center justify-between text-xs text-dark-500">
                      <span>{seriesItem.cast?.join(', ') || 'Cast not available'}</span>
                      <span>{new Date(seriesItem.createdAt).getFullYear()}</span>
                    </div>
                  </div>
                  </motion.div>
                </Link>
          ...
          {series.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="glass-effect rounded-lg p-4 text-center">
                ...
                <div className="text-sm text-dark-400">Total Series</div>
              </div>
              <div className="glass-effect rounded-lg p-4 text-center">
                <div className="text-2xl font-bold text-primary-400 mb-1">
                  {genres.length}
              ...
              <div className="glass-effect rounded-lg p-4 text-center">
                <div className="text-2xl font-bold text-primary-400 mb-1">
                  {series.reduce((sum, series) => sum + series.views, 0).toLocaleString()}
                </div>
