...
export async function POST(request: NextRequest) {
  try {
    ...
    let { data: rawPayments } = await paymentQuery;
    let payments: any[] | null = rawPayments as any[] | null;
    ...
    // Find user
    let { data: rawUsers } = await supabaseServer.from('rahapremium_users').select('*').eq('phone_number', userPhone);
    let users: any[] | null = rawUsers as any[] | null;
    ...
    if (payment.status === 'completed' && payment.game_id) {
      ...
      if (!accesses || (accesses as any[]).length === 0) {
