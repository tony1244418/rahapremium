...
import { ChevronLeft, ChevronRight, Radio, Play, Lock, ExternalLink } from 'lucide-react';
...
export default function LiveTvSlider({ channels, onChannelClick }: LiveTvSliderProps) {
  ...
  const handleChannelClick = (channel: LiveChannel) => {
    if (channel.externalUrl) {
      const isFree = !channel.requiredPackages || channel.requiredPackages.length === 0;
      const userHasAnyPackage = user?.subscription?.isActive === true;
      const hasAccess = isFree || (userHasAnyPackage && hasAccessToContent(user, channel.requiredPackages));
      if (hasAccess) {
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-5 sm:p-8">
            ...
            <div className="flex items-center gap-2 mb-3">
              {channel.externalUrl ? (
                <div className="flex items-center gap-1.5 bg-cyan-600/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-white shadow-lg">
            ...
            <div className="flex items-center gap-3">
              <motion.button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold shadow-xl transition-all duration-200 ${
                  hasAccess
                    ? channel.externalUrl
                      ? 'bg-cyan-600 hover:bg-cyan-500 text-white shadow-cyan-600/40'
