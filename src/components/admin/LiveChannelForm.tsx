...
import { X, Upload, Image as ImageIcon, Radio, DollarSign, ToggleLeft, ToggleRight, ExternalLink } from 'lucide-react';
...
export default function LiveChannelForm({ isOpen, onClose, onSubmit, editData, loading = false }: LiveChannelFormProps) {
  ...
  useEffect(() => {
    if (editData) {
      ...
      if (editData.externalUrl) {
        linkType = 'external';
      } else if (editData.encryptionType === 'clearkey' || hasClearKeys) {
        linkType = 'drm';
      ...
      setFormData({ ...editData, externalUrl: editData.externalUrl || '', linkType, drmKid, drmKey });
    } else {
      setFormData({
        name: '',
        description: '',
  ...
  const validateForm = () => {
    ...
    if (formData.linkType === 'external') {
      const urlToValidate = (formData.externalUrl || formData.streamUrl || '').trim();
      if (!urlToValidate) {
        newErrors.externalUrl = 'External Link URL is required';
      } else {
        try {
          new URL(urlToValidate);
        } catch {
          newErrors.externalUrl = 'Invalid URL format (e.g., https://example.com/live)';
        }
      }
    } else {
    ...
    if (formData.externalUrl && formData.externalUrl.trim()) {
      try {
        ...
        newErrors.externalUrl = 'Invalid external URL format';
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (linkType === 'external') {
        const extUrl = (submitData.externalUrl || submitData.streamUrl || '').trim();
        submitData.externalUrl = extUrl;
        submitData.streamUrl = extUrl;
        submitData.streamFormat = 'other';
        submitData.encryptionType = 'none';
        submitData.clearKeys = {};
      } else if (linkType === 'drm') {
      ...
      if (linkType !== 'external') {
        submitData.externalUrl = submitData.externalUrl?.trim() || null;
      }
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., BBC News, ESPN Sports, Azam Sports"
              />
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setFormData({
                    ...
                    drmKid: '',
                    drmKey: '',
                    ...(lt === 'external' && formData.streamUrl && !formData.externalUrl ? { externalUrl: formData.streamUrl } : {})
                  });
                }}
                className="w-full px-4 py-2 bg-dark-900 border border-dark-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-primary-500"
              >
                <option value="auto">Auto Detect (from Stream URL)</option>
                <option value="external">External Link (Opens external website in new tab on click)</option>
              ...
              <p className="mt-1 text-xs text-gray-500">
                External Link: Opens external URL directly when user clicks &nbsp;|&nbsp;
                HLS/DRM: Plays stream inside web app
              </p>
            </div>

            {/* External Link Settings — shown when External is selected */}
            {formData.linkType === 'external' ? (
              <div className="bg-dark-900 border border-cyan-500/40 rounded-xl p-4 space-y-3">
                <div className="flex items-center gap-2 text-cyan-400 font-semibold text-sm">
                  <ExternalLink className="w-4 h-4" />
                  <span>External Website Link Settings</span>
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, externalUrl: e.target.value, streamUrl: e.target.value })}
                    className="w-full px-4 py-2.5 bg-dark-800 border border-dark-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 font-mono text-sm"
                    placeholder="https://example.com/live-match-or-stream"
                  />
                  {errors.externalUrl && <p className="mt-1 text-xs text-red-400">{errors.externalUrl}</p>}
                </div>
                <p className="text-xs text-gray-400">
                  When a user clicks on this channel in Live TV, it will automatically open this external URL in a new tab.
                </p>
              </div>
            ) : (
              /* Stream URL for normal streams */
              <div className="space-y-4">
                ...
                {/* Optional External Link for fallback / direct open */}
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1 flex items-center gap-1.5">
                    <ExternalLink className="w-3.5 h-3.5 text-gray-400" />
                    External Link (Optional — overrides stream player to open external URL)
                  </label>
                  <input
                    type="url"
                    value={formData.externalUrl || ''}
                    onChange={(e) => setFormData({ ...formData, externalUrl: e.target.value })}
                    className="w-full px-3 py-2 bg-dark-900 border border-dark-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 font-mono text-xs"
                    placeholder="https://example.com/external-player"
                  />
                  {errors.externalUrl && <p className="mt-1 text-xs text-red-400">{errors.externalUrl}</p>}
                </div>
              </div>
