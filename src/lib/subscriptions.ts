...
export const initiateGamePayment = async (
  ...
  const { data: docRef, error } = await supabase.from('payments').insert(paymentRequest).select().single();
  if (error) {
    console.error('Supabase game payment insert error:', error.code, error.message, error.details);
    throw new Error(`Database error (${error.code}): ${error.message}`);
  }
...
export const initiateContentPayment = async (
  ...
  const { data: docRef, error } = await supabase.from('payments').insert(paymentRequest).select().single();
  if (error) {
    console.error('Supabase content payment insert error:', error.code, error.message, error.details);
    throw new Error(`Database error (${error.code}): ${error.message}`);
  }
