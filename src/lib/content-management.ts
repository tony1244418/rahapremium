...
const mapMovie = (data: any): Movie => ({
  ...
  videoUrl: data.video_url || data.videoLink || '',
  downloadUrl: data.download_url || data.downloadUrl || '',
  googleDriveUrl: data.google_drive_url || data.googleDriveUrl || '',
  thumbnailUrl: data.thumbnail_url || data.thumbnailUrl || '',
  duration: data.duration,
  releaseDate: data.release_date || data.releaseDate || data.releaseYear ? new Date(data.release_date || data.releaseDate || data.releaseYear) : null,
  genre: data.genre || [],
  language: data.language || 'sw',
  quality: data.quality ? (Array.isArray(data.quality) ? data.quality : [data.quality]) : [],
  requiredPackages: data.required_packages || data.requiredPackages || [],
  createdAt: data.created_at || data.createdAt ? new Date(data.created_at || data.createdAt) : new Date(0),
  updatedAt: data.updated_at || data.updatedAt ? new Date(data.updated_at || data.updatedAt) : new Date(0),
  views: data.views || 0,
  isActive: data.is_active ?? data.isActive ?? true,
  isAdult: data.is_adult ?? data.isAdult ?? false,
  adultCategory: data.adult_category || data.adultCategory || null,
  rating: data.rating || 0,
  cast: data.cast_list || data.cast || [],
  director: data.director || '',
  searchKeywords: data.search_keywords || data.searchKeywords || [],
  contentPurchaseEnabled: data.content_purchase_enabled ?? data.contentPurchaseEnabled ?? false,
  contentPrice: data.content_price ?? data.contentPrice ?? 0,
  contentPriceDays: data.content_price_days ?? data.contentPriceDays ?? 30,
  contentPurchasePackages: data.content_purchase_packages || data.contentPurchasePackages || [],
  videoEmbedCode: data.video_embed_code || data.videoEmbedCode || ''
});
