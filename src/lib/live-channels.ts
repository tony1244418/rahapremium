...
const mapChannel = (row: any): LiveChannel => ({
  ...
  externalUrl: row.external_url || row.external_link || undefined,
});
...
export const addLiveChannel = async (channelData: Omit<LiveChannel, 'id' | 'createdAt' | 'updatedAt' | 'viewerCount' | 'totalViews'>) => {
  try {
    ...
    const { data, error } = await supabase.from('live_channels').insert({
      ...
      external_url: channelData.externalUrl || null,
...
export const updateLiveChannel = async (channelId: string, channelData: Partial<LiveChannel>) => {
  try {
    ...
    if (channelData.externalUrl !== undefined) updateData.external_url = channelData.externalUrl || null;
...
export const reorderChannelsAlphabetically = async () => {
  try {
    ...
    const channels: LiveChannel[] = result.data;
    const sortedNames = channels.map((c: LiveChannel) => c.name.toLowerCase()).sort();

    await Promise.all(channels.map((channel: LiveChannel) => {
...
export const searchLiveChannels = async (searchQuery: string, includeAdult: boolean = false) => {
  try {
    ...
    const filtered = (result.data as LiveChannel[]).filter((channel: LiveChannel) => {
      const nameMatch = channel.name.toLowerCase().includes(q);
      const descriptionMatch = channel.description?.toLowerCase().includes(q);
      const keywordMatch = channel.searchKeywords?.some((kw: string) => kw.toLowerCase().includes(q));
      const categoryMatch = Array.isArray(channel.category) && channel.category.some((cat: string) => cat.toLowerCase().includes(q));
