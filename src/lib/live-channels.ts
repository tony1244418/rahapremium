...
export const addLiveChannel = async (channelData: Omit<LiveChannel, 'id' | 'createdAt' | 'updatedAt' | 'viewerCount' | 'totalViews'>) => {
  try {
    ...
    const allChannels = await getLiveChannels(false, true);

    // If the admin didn't set an explicit order (or left it at the default 0),
    // place the new channel at the END of the list so it doesn't bump others.
    const maxOrder = (allChannels.data || []).reduce((max, ch) => Math.max(max, ch.order ?? 0), 0);
    const order = (channelData.order !== undefined && channelData.order !== 0)
      ? channelData.order
      : maxOrder + 10;
...
// Update display_order for a single channel (used by quick-order inline edit)
export const updateChannelOrder = async (channelId: string, newOrder: number) => {
  try {
    const { error } = await supabase.from('live_channels').update({
...
// Fix channels that all share display_order = 0 by assigning sequential order based on DB row position
export const fixDuplicateChannelOrders = async () => {
  try {
    ...
    const allZero = channels.every(c => (c.order ?? 0) === 0);
    if (!allZero) return { success: true }; // already has proper ordering

    // Assign sequential order to all channels (keep current DB order as baseline)
    await Promise.all(channels.map((channel, index) =>
      supabase.from('live_channels').update({
    ...
    console.error('Error fixing duplicate channel orders:', error);
    return { success: false, error: error instanceof Error ? error.message : JSON.stringify(error) };
  }
};
...
export const reorderChannelsAlphabetically = async () => {
  try {
    ...
    // Sort channels alphabetically by name (stable, preserves insertion order for ties)
    const sorted = [...result.data].sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: 'base' }));

    // Assign each channel a unique order value based on its sorted position
    await Promise.all(sorted.map((channel, index) => {
